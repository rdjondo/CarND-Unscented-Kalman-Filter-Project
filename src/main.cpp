...
void readInputFile(ifstream& in_file_,
...
void setOutputFileColumnName(const ofstream& out_file_) {
...
void outputToFile(const ofstream& out_file_,
		const vector<MeasurementPackage>& measurement_pack_list, size_t k,
		const UKF& ukf, const vector<GroundTruthPackage>& gt_pack_list) {
	// timestamp
...
void closeOutputFile(const vector<VectorXd>& estimations,
		const vector<VectorXd>& ground_truth, ofstream out_file_,
		ifstream in_file_) {
...
int main(int argc, char* argv[]) {
  ...
  // prep the measurement packages (each line represents a measurement at a
  // timestamp)
  readFile(in_file_, measurement_pack_list, gt_pack_list);
  ...
  // column names for output file
  setOutputFileColumnName(out_file_);
  ...
  for (size_t k = 0; k < number_of_measurements; ++k) {
    ...
    // timestamp
	outputToFile(out_file_, measurement_pack_list, k, ukf, gt_pack_list);
  ...
  // compute the accuracy (RMSE)
  closeOutputFile(estimations, ground_truth, out_file_, in_file_);
  cout << "Done!" << endl;
  return 0;
}
